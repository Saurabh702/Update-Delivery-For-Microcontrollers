...
// When a character is received by the PC UART WaitCommEvent() returns and the received string is read- //
// -using ReadFile(); function.The characters are then displayed on the Console.                      //
//----------------------------------------------------------------------------------------------------//
// BaudRate     -> 9600                                                                               //
// Data format   -> 8 databits,No parity,1 Stop bit (8N1)                                              //
// Flow Control -> None                                                                               //
//----------------------------------------------------------------------------------------------------//
...
string readSerial(char *COMport)
{
    HANDLE hComm;                          // Handle to the Serial port
    char *Prefix = "\\\\.\\";              // For COM ports other than COM0-COM9
    char *ComPortName = string_concat((char *[])
    {
        Prefix,COMport
    },2).ptr;                              // Name of the Serial port(May Change) to be opened
    BOOL  Status;                          // Status of the various operations
    DWORD dwEventMask;                     // Event mask to trigger
    char  TempChar;                        // Temporary Character
    string serialBuffer;
    init_string(&serialBuffer,0);         // Buffer Containing Rxed Data
    ...
    */
    ...
    else //If  WaitCommEvent()==True Read the RXed data using ReadFile();
    {
        ...
        do
        {
            ...
            {
                serialBuffer.ptr[i] = EOS;
                break;
            }
            serialBuffer.ptr[i] = TempChar;
            i++;
        }
        while (NoBytesRead > 0);
        ...
        //printf("\n\n    ");
        //int j =0;
        //for (j = 0; j < i-1; j++)		// j < i-1 to remove the duplicated last character
